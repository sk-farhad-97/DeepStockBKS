...
def reward_function_strategy(action, trade_info, time_step, stock_slot):
    # print('strategy.....')
    ...
    if action == 0:
        reward = ((close_cur - close_pre) * stock_slot) / cash_pre # unrealized PNL
    elif action == 1:
        reward = ((close_cur - buy_pre) * stock_slot) / cash_pre # realized PNL
    elif action == 2:
        if buy_pre>0:
            reward = (-buy_pre * 0.25 * stock_slot) / cash_pre
        else:
            reward = 0
    else:
        '''Forbidden action'''
